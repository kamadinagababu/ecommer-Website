...
//         <div className=" px-10 text-gray-600 mx-auto flex flex-wrap  flex-col md:flex-row items-center shadow-xl sticky top-0 bg-white z-10">
//           <a
//             href="/"
...
const Navbar = ({
  ...
  return (
    <header className="body-font fixed w-full z-10 bg-white shadow-xl top-0">
      <div className="container mx-auto flex flex-wrap items-center justify-between p-4">
        <Link href="/" legacyBehavior>
          <a className="flex items-center mb-4 md:mb-0">
            <Image src={logo} alt="Logo" width={60} height={40} />
            <span
              className="ml-3 text-xl text-gray-900"
              style={{ color: "#0097b2", fontFamily: "Georgia" }}
            >
              AmiKart
            </span>
        ...
        <div className="flex items-center space-x-4">
          {user && user.value && (
            <div
              ...
              className="relative"
            >
              <button className="rounded-full w-12 h-12 bg-gray-200 p-3 border-0 inline-flex items-center justify-center text-gray-500">
                <i className="fa-regular fa-user w-5 h-5"></i>
              </button>
              {dropdown && (
                <div className="absolute top-14 right-0 rounded-md px-5 w-48 bg-indigo-500">
                  <div className="px-4 py-4 text-sm text-gray-900 dark:text-white font-medium truncate">
                    {localStorage.getItem("name")}
                    <div className="font-medium truncate">
                      {localStorage.getItem("email")}
                    </div>
                  ...
                  <Link href="/myaccount" legacyBehavior>
                    <a className="block py-1 dark:text-white hover:text-indigo-200 text-sm">
                      My Account
                    </a>
                  </Link>
                  <Link href="/orders" legacyBehavior>
                  ...
                  <button
                    onClick={logout}
                    className="block py-1 pb-3 dark:text-white hover:text-indigo-200 text-sm cursor-pointer"
                  >
                    Logout
                  </button>
                </div>
          ...
          {!user && (
            <Link href="/login" legacyBehavior>
              <a
                className="cursor-pointer inline-flex items-center bg-gray-100 border-0 py-2 px-4 focus:outline-none hover:bg-gray-200 rounded text-base"
      ...
      <div
        ref={ref}
        className={`w-64 sm:w-80 md:w-96 lg:w-1/3 xl:w-1/4 h-full sidecart fixed top-0 right-0 bg-blue-100 py-10 px-8 transition-transform transform ${
      ...
      >
        ...
        <ol className="list-decimal">
          {Object.keys(cart).length === 0 && (
            <div className="my-4 text-xl text-center">Cart is Empty!</div>
          )}
          {Object.keys(cart).map((k) => (
            <li key={k} className="flex items-center justify-between my-5">
              ...
              <div className="w-1/3 flex items-center justify-center">
                <i
                  ...
                  className="fa-solid fa-square-minus cursor-pointer mx-1"
                ></i>{" "}
                {cart[k].qty}{" "}
                <i
                  onClick={() =>
                  ...
                  className="fa-solid fa-square-plus cursor-pointer mx-1"
                ></i>
              </div>
            </li>
          ))}
        ...
        <div className="flex flex-col items-center space-y-4">
          <Link href="/checkout" legacyBehavior>
            <a
              disabled={disable}
              className="text-white disabled:bg-indigo-400 flex mx-2 bg-indigo-600 border-0 py-2 px-3 focus:outline-none hover:bg-indigo-700 rounded text-lg w-full text-center"
            >
              <span>
                <i className="fa-solid fa-bag-shopping mr-3"></i>
              </span>
          ...
          <button
            onClick={ClearCart}
            disabled={clear}
            className="text-white flex mx-2 disabled:bg-indigo-400 bg-indigo-600 border-0 py-2 px-4 focus:outline-none hover:bg-indigo-700 rounded text-lg w-full"
          >
            Clear
          </button>
        </div>
